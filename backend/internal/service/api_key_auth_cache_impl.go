...
const apiKeyAuthSnapshotVersion = 25 // v25: opt-in ordered group bindings
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		GroupBindingsEnabled: apiKey.GroupBindingsEnabled,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		GroupBindingsEnabled: snapshot.GroupBindingsEnabled,
		GroupBindings:        snapshot.GroupBindings,
		Key:                  key,
		Name:                 snapshot.Name,
